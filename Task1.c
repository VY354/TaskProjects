...
#include <stdio.h>
//Подключаем математическую библиотеку для использования числа пи и 
//функци возведения в степень
#include <math.h>
//Подключаем стандартную библиотеку для использования команды clear
#include <stdlib.h>
//Создаем функцию Culcexpr для проведения вычесления с заданной точностью. 
//Принимает float x и int accr
...
void CulcExpr(float x,int accr){
	...
	int i,Count=0;
	//Объявляем переменные для хранения значения выражения данного слева 
	//от знака равно и ряда справа
	float expr1,expr2;
	//Вычисляем выражение, стоящее слева от равно
	expr1=M_PI*M_PI/8-M_PI/4*fabs(x);
	//начинаем цикл от 1 до +inf с шагом 2, т.к. множитель 2n+1 будет принимать 
	//значения 3,5,7,9...
	for(i=1;1;i+=2){
		/*Проверка на точность. 10^-accr есть граница точность. 
		Т.е. например, 10^-2 - это вычисление с точностью до сотых.
		Когда разность между исходным выражением и 
		рядом станет меньше заданной точности, цикл прервется
		*/
	...
	//Вывод результатов
	//Значение х
	printf("Value of right expression     | %f\n",expr2);
	printf("Value of left  expression     | %f\n",expr1);
	printf("-----------------------------------\n");
