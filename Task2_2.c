...
int main(){
	...
	printf("Y ~ 0 when x = %f\n",FindRoot(a,b,x_n,accr,choose));
	//так как функция main типа int, то она должна что-то возвращать
	return 0;
}
//Реализация функции для нахождения корня
float FindRoot(float a,float b,float x_n,int accr,int choose){
	//Задаем счетчик максимального количества итераций (static, чтобы по выхлду из функции значение сохранялось)
	static int i;
	//Задаем переменные для хранения точки пересечения касательной а осью Ох, значения выражения и значения производной
	float x_n_additional,x_n1=0,expr,der;
	//Запоминаем текущий x_n, так как в строчке 98 x_n становится равным x_n1 и их разность соответственно 0.
	x_n_additional=x_n;
	...
	//вызываем функцию для проверки знаков на отрезке [ x_n-pow(10,-accr) ; x_n1+pow(10,-accr) ]
	if(CheckSign(a,b,x_n,x_n1,accr,choose))
		//Если возвращается 1, то продолжаем вычисления с тчностью, увеличенной на 1 порядок
		FindRoot(a,b,x_n1,accr+1,choose);
	//Если CheckSign возвращает 0, то завершаем программу и возвращам значение корня
	return x_n1;
}
...
//Реализация функции для проверки знаков функции на краях отрезка [ x_n-pow(10,-accr) ; x_n1+pow(10,-accr) ]
_Bool CheckSign(float a,float b,float x_n,float x_n1,int accr,int choose){
	//Объявляем переменные для хранения значения функции на кончах отрезка [ x_n-pow(10,-accr) ; x_n1+pow(10,-accr) ]
